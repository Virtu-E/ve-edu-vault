...
class DjangoDbToolConf(ToolConfAbstract):
    ...
    def get_lti_tool(self, iss, client_id):
        client_id = "1e399a69-10e8-40bb-8e86-a030c8b4d065"
    ...
    def find_registration_by_params(self, iss, client_id, *args, **kwargs):
        ...
        reg = Registration()
        reg.set_auth_login_url(lti_tool.auth_login_url).set_auth_token_url(lti_tool.auth_token_url).set_auth_audience(auth_audience).set_client_id(lti_tool.client_id).set_key_set(key_set).set_key_set_url(key_set_url).set_issuer(lti_tool.issuer).set_tool_private_key(lti_tool.tool_key.private_key).set_tool_public_key(tool_public_key)
        return reg
