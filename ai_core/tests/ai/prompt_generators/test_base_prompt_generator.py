...
from ai_core.learning_mode_rules import BaseLearningModeRule, NormalRule, CalculatorLearningMode
...
def learning_history(user, topic):
    return LearningHistory(userId=user.id, block_id=topic.block_id, modeHistory={"normal": [ModeDataFactory()]})


@pytest.fixture
def config():
    return QuestionPromptGeneratorConfig(course_name="Mathematics", topic_name="Algebra", difficulty="medium", academic_level="High School", syllabus="Basic Algebra", category="Math")
...
class TestBaseQuestionPromptGenerator:
    def test_init_with_valid_data(self, learning_history, repository, config):
        rule = NormalRule()
        generator = BaseQuestionPromptGenerator(rule=rule, learning_history=learning_history, repository=repository, question_ids=["1", "2"], config=config, failed_difficulties=["easy", "medium"])
    ...
    def test_validate_prerequisite_with_missing_prerequisite(self, learning_history, repository, config):
        ...
        with pytest.raises(ValueError) as exc_info:
            BaseQuestionPromptGenerator(rule=TestRule(), learning_history=learning_history, repository=repository, question_ids=["1", "2"], config=config, failed_difficulties=["easy"])
    ...
    def test_get_failed_tags(self, learning_history, repository, config):
        rule = NormalRule()
        generator = BaseQuestionPromptGenerator(rule=rule, learning_history=learning_history, repository=repository, question_ids=["1", "2"], config=config, failed_difficulties=["easy"])
