...
def mock_dependencies():
    ...
    return {"question_filter": Mock(), "attempt_calculator": Mock(), "time_analyzer": Mock(), "completion_analyzer": Mock(), "tags_collector": Mock()}


@pytest.fixture
def calculator(mock_dependencies):
    """Fixture providing an instance of DifficultyStatsCalculator with mocked dependencies."""
    return DifficultyStatsCalculator(mock_dependencies["question_filter"], mock_dependencies["attempt_calculator"], mock_dependencies["time_analyzer"], mock_dependencies["completion_analyzer"], mock_dependencies["tags_collector"])
...
class TestDifficultyStatsCalculator:
    ...
    def test_calculate_with_valid_data(self, calculator, mock_dependencies):
        ...
        mock_dependencies["attempt_calculator"].calculate_attempt_specific_rates.return_value = {"first": 0.6, "second": 0.3, "third": 0.1}

        mock_dependencies["time_analyzer"].analyze_time.return_value = {"averageFirstAttemptTime": 120.0, "averageSecondAttemptTime": 90.0, "averageThirdAttemptTime": 60.0, "firstAttempt": 120.0, "secondAttempt": 90.0, "thirdAttempt": 60.0}

        mock_dependencies["completion_analyzer"].analyze_completion.return_value = {"completionRate": 0.75, "incompleteRate": 0.15, "earlyAbandonment": 0.10}

        mock_dependencies["tags_collector"].collect_failed_tags.return_value = ["arrays", "sorting"]
...
class TestQuestionFilter:
    def test_filter_by_difficulty(self):
        ...
        questions = [QuestionAIContextFactory(difficulty="easy"), QuestionAIContextFactory(difficulty="medium"), QuestionAIContextFactory(difficulty="hard"), QuestionAIContextFactory(difficulty="medium")]
    ...
    def test_filter_by_difficulty_no_matches(self):
        ...
        filter_instance = QuestionFilter()
        questions = [QuestionAIContextFactory(difficulty="easy"), QuestionAIContextFactory(difficulty="medium")]
