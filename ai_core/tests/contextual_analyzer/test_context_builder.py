...
class TestQuestionContextBuilder:
    ...
    def test_build_question_context_successful(self):
        ...
        result = self.question_context_builder.build_question_context(
            question_ids, self.sample_metadata
        )
    ...
    def test_build_question_context_missing_metadata(self):
        ...
        assert (
            str(exc_info.value)
            == f"Invalid parameter: 'Question {self.q2_id} not found in attempt data'"
        )
    ...
    def test_build_question_context_with_different_attempt_data(self):
        ...
        question = QuestionFactory(
            _id=self.q1_id, question_id=self.q1_id, difficulty="easy", tags=["math"]
        )
        metadata = {
            self.q1_id: QuestionMetadataFactory(
                is_correct=False,
    ...
    def test_build_question_context_with_none_metadata(self):
        ...
        assert (
            str(exc_info.value)
            == f"Invalid parameter: 'Question {self.q1_id} not found in attempt data'"
        )
    ...
    def test_build_question_context_with_none_question_ids(self):
        ...
        with pytest.raises(TypeError):
            self.question_context_builder.build_question_context(
                question_ids, self.sample_metadata
            )
    ...
    def test_build_question_context_with_duplicate_question_ids(self):
        ...
        assert (
            len(result) == 1
        )  # Should only return one context even with duplicate IDs
        assert result[0].id == self.q1_id
    ...
    def test_build_question_context_with_empty_tags(self):
        ...
        result = self.question_context_builder.build_question_context(
            [self.q1_id], self.sample_metadata
        )
