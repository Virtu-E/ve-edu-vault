...
def mock_difficulty_stats():
    return {
        "easy": DifficultyStats(
            ...
            successRate=0.8,
            averageTime=120.0,
            failedTags=["algebra", "geometry"],
            firstAttemptSuccessRate=0.7,
            secondAttemptSuccessRate=0.8,
            thirdAttemptSuccessRate=0.9,
            averageAttemptsToSuccess=1.5,
            completionRate=0.9,
            incompleteRate=0.1,
            earlyAbandonment=0.05,
            averageFirstAttemptTime=100.0,
            averageSecondAttemptTime=90.0,
            averageThirdAttemptTime=80.0,
            timeDistribution={"0-5min": 0.3, "5-10min": 0.7},
        )
    }
...
def mock_builders(mock_difficulty_stats):
    ...
    question_contexts = [
        QuestionAIContext(
            ...
            difficulty="easy",
            tags=["algebra"],
            attempts=Attempt(success=True, timeSpent=120, attemptNumber=1),
        )
    ]
...
def context_setup(user, mock_repositories, mock_builders, topic):
    ...
    return {
        ...
        "repositories": (question_repo, learning_history_repo),
        "builders": (context_builder, stats_calculator),
...
class TestContextEngine:
    def test_generate_learning_history_success(self, context_setup):
        ...
        context_builder.build_question_context.assert_called_once_with(
            question_set.question_list_ids,
            question_attempt.get_latest_question_metadata,
        )
